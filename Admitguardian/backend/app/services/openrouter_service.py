...
async def analyze_essay(document_text: str) -> dict:
    ...
    Returns a response matching EssayAnalysisResponse with the following fields:
    - risk_score
    - strengths
    - weaknesses
    - red_flags
    ...
    {{
        ...
        "suggestions": [...],
        "red_flags": [...],  # added red_flags to match the required structure
    }}
    ...
    try:
        ...
        if extracted_json:
            # Prepare the final response matching the EssayAnalysisResponse model
            response = {
                "risk_score": int(extracted_json.get("overall_score", "0 / 10").split('/')[0].strip()),  # Extract score from "overall_score"
                "strengths": extracted_json.get("strengths", []),
                "weaknesses": extracted_json.get("weaknesses", []),
                "red_flags": extracted_json.get("red_flags", []),  # Default to empty if missing
                "suggested_improvements": extracted_json.get("suggestions", []),  # Map suggestions correctly
            }
            return response
        else:
